...
def main():
        ...
        with st.form("Add New Model Form", clear_on_submit=True):
            ...
            if st.form_submit_button("Add New Model"):
                if new_model and selected_use_case:
                    ...
                    st.success(
                        "New model and prompt added successfully with versioning!"
                    )
                    st.rerun()
                else:
    ...
    with tabs[2]:
        ...
        if selected_model and selected_use_case:
            prompts = asyncio.run(
                fetch_api(f"/view_prompts/{selected_model}/{selected_use_case}")
            )
            if prompts:
    ...
    with tabs[4]:
        with st.form("Add Assistant Form", clear_on_submit=True):
            ...
            assistant_apikey = st.text_input(
                "Assistant API Key", key="assistant_apikey"
            )
            assistant_version = st.text_input(
                "Assistant Version", key="assistant_version"
            )
            use_cases = asyncio.run(fetch_api("/fetch_use_cases"))
            selected_use_case = st.selectbox(
            ...
            if st.form_submit_button("Add Assistant"):
                if (
                    assistant_id
                    and assistant_apikey
                    and assistant_version
